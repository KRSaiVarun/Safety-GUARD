...
import { sendEmergencyAlert, sendSafeNotification, getFreshLocation } from '@/lib/alerts'
...
export function useEmergencySession() {
  const {
    ...
    updateState, addTimelineEvent, updateRiskScore,
    setAlertDelivery, resetAlertDelivery,
  } = useSessionStore()
  ...
  // ── Fire one alert ────────────────────────────────────────────────────────
  const fireAlert = useCallback(async (sessionId: string, isRepeat = false) => {
    ...
    addTimelineEvent({
      ...
      data: {
        ...
        lat,
        lng,
        maps_link: result.maps_link ?? (lat && lng ? `https://www.google.com/maps?q=${lat},${lng}` : null),
        error:     result.error,
      },
  ...
  const startRepeatedAlerts = useCallback((sessionId: string) => {
  ...
  const triggerEmergency = useCallback(async (
    ...
    // Notify admin dashboards immediately
    import('@/lib/socket').then(mod => {
      mod.default.emit('EMERGENCY_TRIGGERED', {
        sessionId: session.id,
        userId,
        method,
        startedAt: session.startedAt,
      })
      console.log('[SOCKET] Emitted EMERGENCY_TRIGGERED')
    })
    ...
    startSession, resetAlertDelivery, startTracking,
    startRiskScoring, updateState, addTimelineEvent,
    fireAlert, startRepeatedAlerts,
  ])
  ...
  const resolveEmergency = useCallback(async () => {
    ...
    setAlertDelivery({ status: result.ok ? 'sent' : 'failed', error: result.error })
    addTimelineEvent({
  ...
  }, [currentSession, stopTracking, endSession, setAlertDelivery, addTimelineEvent])
