...
import type { AlertDelivery } from '@/lib/alerts'
import { supabase } from '@/lib/supabase'

const IDLE_DELIVERY: AlertDelivery = {
  status: 'idle',
  count: 0,
  lastSentAt: null,
  lastAttempt: 0,
...
interface SessionStore {
  ...
  clearSession: () => void
  setAlertDelivery: (d: Partial<AlertDelivery>) => void
  resetAlertDelivery: () => void
}
...
export const useSessionStore = create<SessionStore>()(
  persist(
    (set, get) => ({
      ...
      setAlertDelivery: (d) =>
        set(s => ({ alertDelivery: { ...s.alertDelivery, ...d } })),

      resetAlertDelivery: () => set({ alertDelivery: IDLE_DELIVERY }),
    }),
