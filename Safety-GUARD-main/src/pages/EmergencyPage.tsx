  ...
  if (status === "sent")
    return (
      <div className={styles.alertStatus} data-status="sent">
        <CheckCircle size={14} />✅ WHATSAPP ALERT SENT
      </div>
    );
  return (
    <div className={styles.alertStatus} data-status="failed">
      <XCircle size={14} />❌ ALERT DELIVERY FAILED — {error ?? "check backend"}
    </div>
  );
}
...
export default function EmergencyPage() {
  ...
  const { currentSession, triggerEmergency, resolveEmergency, getAlertCount } =
    useEmergencySession();
  ...
  useEffect(() => {
    if (phase !== "active") return;
    alertPollRef.current = globalThis.setInterval(() => {
      setAlertCount(getAlertCount());
    }, 500);
    setNextAlertIn(30);
    nextAlertRef.current = globalThis.setInterval(() => {
  ...
  return (
    <div
      className={`${styles.page} ${phase === "active" ? styles.pageActive : ""}`}
    >
      {/* Top bar */}
      <div className={styles.topBar}>
        <div className={styles.topLeft}>
          <Shield
            size={20}
            style={{
              color: phase === "active" ? "var(--red)" : "var(--text-2)",
            }}
          />
          <span
            className={`${styles.status} ${phase === "active" ? styles.statusActive : ""}`}
          >
            {phase === "standby"
              ? "SAFETY GUARD — STANDBY"
              : phase === "countdown"
                ? "ACTIVATING…"
                : "⚠ EMERGENCY ACTIVE"}
          </span>
        ...
        {phase === "standby" && (
          <>
            <p className={styles.instruction}>
              Hold the button to send emergency alert to your contacts
            </p>
          ...
          <>
            ...
            <div className={styles.activeStats}>
              ...
              <div className={styles.activeStat}>
                ...
                <span
                  className={styles.activeStatVal}
            ...
            {!resolved && (
              <div className={styles.nextAlertRow}>
                ...
                <span className={styles.nextAlertLabel}>
                  NEXT WHATSAPP ALERT IN
                </span>
              ...
              <div className={styles.timeline}>
                {currentSession.timeline.slice(-4).map((ev) => (
                  <div key={ev.id} className={styles.timelineItem}>
                    <span
                      className={`${styles.timelineDot} ${ev.eventType === "ALERT_SENT" || ev.eventType === "ALERT_REPEATED" ? styles.timelineDotGreen : ""}`}
                    />
            ...
            {!resolved && (
              <div className={styles.pinSection}>
                ...
                <div
                  className={`${styles.pinRow} ${pinShake ? styles.pinShake : ""}`}
                >
